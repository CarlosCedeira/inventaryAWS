  ...
  try {
    const [rows] = await connection.execute(`
      SELECT i.id AS inventario_id, i.tenant_id, i.producto_id,
             p.nombre AS producto_nombre, c.nombre AS producto_categoria,
             c.id AS categoria_id, i.cantidad, i.precio_compra, i.precio_venta,
             i.stock_minimo, i.fecha_caducidad, p.publicado, p.variantes
      FROM inventario i
  ...
  try {
    const [rows] = await connection.execute(
      ...
      SELECT i.id AS inventario_id, i.tenant_id, i.producto_id,
             ...
             p.publicado, p.destacado, p.recomendado, p.ranking, p.fecha_creacion,
             p.variantes, c.id AS categoria_id, c.nombre AS producto_categoria,
             i.cantidad, i.stock_minimo, i.precio_compra, i.precio_venta,
             i.fecha_caducidad, i.codigo_barras, i.numero_lote, i.sku, i.ultima_actualizacion
      FROM inventario i
      INNER JOIN productos p ON i.producto_id = p.id
      LEFT JOIN categorias c ON p.categoria_id = c.id
      WHERE i.id = ?;
...
async function updateProduct(inventarioId, productoData, inventarioData) {
  ...
  try {
    ...
    await connection.execute(
      `UPDATE inventario
       SET cantidad = ?, stock_minimo = ?, precio_compra = ?, precio_venta = ?, fecha_caducidad = ?, 
           codigo_barras = ?, numero_lote = ?, sku = ?, ultima_actualizacion = NOW()
       WHERE id = ?`,
      [
